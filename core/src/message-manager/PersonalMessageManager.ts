    ...
    return new Promise((resolve, reject) => {
      ...
      this.hub.once(`${messageId}:finished`, (data: PersonalMessage) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(new Error('Vista Wallet Personal Message Signature: User denied message signature.'));
          default:
            return reject(
              new Error(`Vista Wallet Personal Message Signature: Unknown problem: ${JSON.stringify(messageParams)}`),
            );
        }
      });
    });
